...
function BuscarCodigo(e) {
    if (e.which == 13) {
        const nombre = document.getElementById("buscar_nombre").value; // Obtener el nombre del producto
        const url = document.getElementById("url").value;
        const urls = url + "Compras/buscarPorNombre"; // Cambiar la URL del controlador
        $.ajax({
            url: urls,
            type: 'POST',
            data: {
                nombre // Enviar el nombre del producto en lugar del código
            },
            success: function (response) {
                if (response != 0) {
...
function BuscarProductoPorNombre(event) {
    if (event.which == 13) {
        ...
        const url = document.getElementById("url").value;
        const urls = url + "Ventas/buscarPorNombre";
        $.ajax({
            url: urls,
            type: 'POST',
            data: {
